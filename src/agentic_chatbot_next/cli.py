    ...
    if check_db:
        try:
            ...
            with psycopg2.connect(dsn=settings.pg_dsn, connect_timeout=max(1, int(timeout_seconds))) as conn:
                ...
                from agentic_chatbot_next.rag import build_kb_coverage_status  # noqa: PLC0415
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        WHERE tenant_id = %s
                          ...
                          AND source_type = 'kb'
                        """,
                        (settings.default_tenant_id, settings.default_collection_id),
                    )
                    kb_status = build_kb_coverage_status(
                        settings,
                        [
                            {"source_path": row[0], "title": row[1]}
                ...
                missing_source_paths = list(kb_status.missing_source_paths)
                if missing_source_paths:
                    ...
                    checks.append(
                        DoctorCheckResult(
                            ...
                            remediation=f"Run `{kb_status.suggested_fix}` to index KB and docs sources.",
                        )
                    )
                else:
                    checks.append(
                        DoctorCheckResult(
                            ...
                            status="PASS",
                            details=(
                                f"Indexed KB covers all {len(kb_status.configured_source_paths)} configured source files "
