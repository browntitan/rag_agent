...
def _infer_executor(text: str) -> str:
    lower = text.lower()
    if any(token in lower for token in ("csv", "excel", "spreadsheet", "dataframe", "dataset", "pandas")):
        return "data_analyst"
    if (
        ...
        return "utility"
    if any(token in lower for token in ("document", "contract", "policy", "clause", "requirement", "kb", "knowledge base", "upload", "compare")):
        return "rag_worker"
    return "general"
...
def build_fallback_plan(query: str, *, max_tasks: int = 8) -> List[Dict[str, Any]]:
    """Deterministic plan builder used when planner JSON is missing or invalid."""
    lower = query.lower()
...
def select_execution_batch(
    ...
    """Return the next ready task batch with deterministic sequential/parallel semantics."""
